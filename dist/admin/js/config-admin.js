...
export async function loadConfig() {
  ...
  if (fuente) {
    ...
    if (fuente.coords_lat && fuente.coords_lng) {
      ...
      if (hint) hint.textContent = `📍 ${fuente.coords_lat.toFixed(5)}, ${fuente.coords_lng.toFixed(5)}`;
    }
  } else if (cfg) {
    ...
    gpsBtn.addEventListener('click', () => {
      ...
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          if (gpsHint) gpsHint.textContent = `📍 ${newCoords.lat.toFixed(5)}, ${newCoords.lng.toFixed(5)}`;
        },
