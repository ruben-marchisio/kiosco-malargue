...
const CAT_EMOJI = { bebidas:'🥤', snacks:'🍫', comidas:'🍽️', panaderia:'🥐', verduleria:'🥦', limpieza:'🧹', otros:'📦' };
...
loginForm.addEventListener('submit', async e => {
  ...
  if (error) { loginError.textContent = 'Email o contraseña incorrectos'; loginError.style.display = 'block'; }
});
...
async function loadDashboard() {
  ...
  const [{ data: prods }, { data: pedidos }] = await Promise.all([
    ...
    supabase.from('pedidos_log').select('monto_total,monto_envio,monto_comision,created_at').gte('created_at', today),
  ]);
...
async function loadStock() {
  ...
  if (!data?.length) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:30px">No hay productos</td></tr>'; return; }
  ...
  tbody.querySelectorAll('[data-toggle]').forEach(input => {
    input.addEventListener('change', async () => {
      await supabase.from('productos').update({ disponible: input.checked }).eq('id', input.dataset.toggle);
    });
  });
...
async function loadPedidos() {
  ...
  const { data } = await supabase.from('pedidos_log').select('*').order('created_at', { ascending: false }).limit(50);

  if (!data?.length) { list.innerHTML = '<p style="text-align:center;color:#888;padding:30px">Aún no hay pedidos registrados</p>'; return; }
...
pedidoForm.addEventListener('submit', async e => {
  ...
  const monto_comision = es_tercero ? +(monto_total * 0.10).toFixed(2) : 0;

  await supabase.from('pedidos_log').insert({ monto_total, monto_envio, es_delivery, monto_comision, notas });
  pedidoForm.reset();
